...
public class Doodle extends RelativeLayout implements OnClickListener {
	...
	private void init(Context context) {
		...
		mDrawingBoard.setLayoutParams(new RelativeLayout.LayoutParams(w, h));
	...
	private boolean saveToFile(Bitmap mBitmap) {
		if (saveFile != null) {
			...
			try {
				...
				bos.flush();
				log.d("saveToFile success:" + saveFile.getAbsolutePath());
				return true;
			} catch (Exception e) {
	...
	public static interface OnDoodleListener {
	...
	public boolean setSaveFile(String filePath, boolean force) {
		...
		if (!isSupport) {
			log.w("File is not Support this file, suffix:" + suffix);
			return false;
		}
